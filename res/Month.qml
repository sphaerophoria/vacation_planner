...
Column {
    ...
    MonthGrid {
        ...
        delegate: Rectangle {
            color: {
                if (model.date.getUTCMonth() != grid.month) {
                    return "transparent"
                }
                else if (holidays.map(Number).includes(+model.date)) {
                    return "green"
                }
                else if (fixedVacationDays.map(Number).includes(+model.date)) {
                    return "red"
                }
                else if (vacationDays.map(Number).includes(+model.date)) {
                    return "yellow"
                }
                else {
                    return "transparent"
